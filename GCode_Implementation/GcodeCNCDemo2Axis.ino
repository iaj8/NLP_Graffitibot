...
void line(float newx,float newy) {
  ...
  long dx  = calculateDX(newx, px, newy, py);
  long dy  = calculateDY(newx, px, newy, py);
  int dirx = dx>0?1:-1;
...
long calculateDX(float newx, float px, float newy, float py){
  return sqrt(newx*newx) - sqrt(px*px + py*py);
}

long calculateDY(float newx, float px, float newy, float py){
  return 5;
}
