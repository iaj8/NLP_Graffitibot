...
void moveStepper(long new_coords_0, long new_coords_1) {
  Serial.print("Moving from ");
  Serial.print(px); Serial.print(","); Serial.print(py);
  Serial.print(" to ");
  Serial.print(new_coords_0); Serial.print(","); Serial.print(new_coords_1);
  steppers.runSpeedToPosition(); // Blocks until all are in position
  //for example curr_coords = 0,0 and new_coords = 10,0
  long curr_a = sqrt(pow(x_0+px, 2) + pow(y+py,2));
  long curr_b = sqrt(pow(x_1-px, 2) + pow(y+py,2));
  long new_a = sqrt(pow(x_0+(new_coords_0-px), 2) + pow(y+(new_coords_1-py),2));
  long new_b = sqrt(pow(x_1-(new_coords_0-px), 2) + pow(y+(new_coords_1-py),2));
  Serial.print(" Repositioning lengths from ");
  Serial.print(positions[0]);
  Serial.print(",");
  Serial.print(positions[1]);
  positions[0] = multiplier*abs(new_a-curr_a);
  if (new_b > curr_b){
    positions[1] = -multiplier*abs(new_b-curr_b);
  } else {
    positions[1] = multiplier*abs(new_b-curr_b);
  }
  Serial.print(" to ");
